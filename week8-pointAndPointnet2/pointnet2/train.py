...
LOG_DIR = FLAGS.log_dir             # ./log 日志文件夹
if not os.path.exists(LOG_DIR): os.mkdir(LOG_DIR)
os.system('cp %s %s' % (MODEL_FILE, LOG_DIR)) # bkp of model def
os.system('cp train.py %s' % (LOG_DIR)) # bkp of train procedure
LOG_FOUT = open(os.path.join(LOG_DIR, 'log_train.txt'), 'w')            # 训练日志
...
def train_one_epoch(sess, ops, train_writer):
    ...
    loss_sum = 0
    batch_idx = 0
    while TRAIN_DATASET.has_next_batch():           # 读入一个batch
        ...
        train_writer.add_summary(summary, step)         # 绘图
        pred_val = np.argmax(pred_val, 1)           # 预测值
        correct = np.sum(pred_val[0:bsize] == batch_label[0:bsize])     # 预测正确的值个数
        total_correct += correct
        total_seen += bsize
        loss_sum += loss_val
        if (batch_idx+1)%50 == 0:       # 50个batch打印一次截止到当前batch的loss和acc
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    while TEST_DATASET.has_next_batch():
        ...
        test_writer.add_summary(summary, step)      # 绘图
