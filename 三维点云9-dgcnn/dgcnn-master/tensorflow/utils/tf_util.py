...
def pairwise_distance(point_cloud):
  ...
  point_cloud = tf.squeeze(point_cloud)             # 挤掉d=1的维度
  if og_batch_size == 1:            # 如果batch=1，则扩一个维度
    point_cloud = tf.expand_dims(point_cloud, 0)
    
  point_cloud_transpose = tf.transpose(point_cloud, perm=[0, 2, 1])    # 点云转置
  point_cloud_inner = tf.matmul(point_cloud, point_cloud_transpose)    # B*N*N
  point_cloud_inner = -2*point_cloud_inner
  point_cloud_square = tf.reduce_sum(tf.square(point_cloud), axis=-1, keep_dims=True) # B*N*1
  point_cloud_square_tranpose = tf.transpose(point_cloud_square, perm=[0, 2, 1])            # 转置
