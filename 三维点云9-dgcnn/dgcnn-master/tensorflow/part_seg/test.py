...
parser.add_argument('--model_path', default='train_results/trained_models/epoch_175.ckpt', help='Model checkpoint path')
...
def pc_normalize(pc):
  ...
  pc = pc - centroid
  m = np.max(np.sqrt(np.sum(pc**2, axis=1)))   # 到(0,0,0)的最长距离
  pc = pc / m
  return pc

def placeholder_inputs():
  pointclouds_ph = tf.placeholder(tf.float32, shape=(batch_size, point_num, 3))   # B*N*3
  input_label_ph = tf.placeholder(tf.float32, shape=(batch_size, NUM_OBJ_CATS))     # B*16
...
def predict():
  ...
  with tf.Session(config=config) as sess:
    ...
    len_pts_files = len(pts_files)      # 2874个测试点云
    for shape_idx in range(len_pts_files):            # 遍历每一个点
      if shape_idx % 100 == 0:
        printout(flog, '%d/%d ...' % (shape_idx, len_pts_files))

      cur_gt_label = on2oid[labels[shape_idx]] # 0/1/.../15     # 当前点云的类别

      cur_label_one_hot = np.zeros((1, NUM_OBJ_CATS), dtype=np.float32)
      cur_label_one_hot[0, cur_gt_label] = 1      # 当前点云的类别 onehot编码
      ...
      # pts:2704*3;seg:2704标签
      pts, seg = load_pts_seg_files(pts_file_to_load, seg_file_to_load, objcats[cur_gt_label])
      ori_point_num = len(seg)

      batch_data[0, ...] = pc_augment_to_point_num(pc_normalize(pts), point_num)    # 1*3000*3

      seg_pred_res = sess.run(seg_pred, feed_dict={         # 返回：1*3000*50
            pointclouds_ph: batch_data,       # 1*3000*3
            input_label_ph: cur_label_one_hot,    # 1*16
            is_training_ph: is_training})

      seg_pred_res = seg_pred_res[0, ...]     # 3000*50

      iou_oids = object2setofoid[objcats[cur_gt_label]]     # [12,13,14,15]
      non_cat_labels = list(set(np.arange(NUM_PART_CATS)).difference(set(iou_oids)))      # 当前点云没有的类别，[1,2,3...,11,16,17,...49]

      mini = np.min(seg_pred_res)
      seg_pred_res[:, non_cat_labels] = mini - 1000     # 3000*50

      seg_pred_val = np.argmax(seg_pred_res, axis=1)[:ori_point_num]    # 2704个点的分割结果

      seg_acc = np.mean(seg_pred_val == seg)   # 这个点云的分割精度
      ...
      mask = np.int32(seg_pred_val == seg)    # 找到正确分割的点置为1，错误的置为0， (2704,)

      total_iou = 0.0
      iou_log = ''
      for oid in iou_oids:
        n_pred = np.sum(seg_pred_val == oid)   # 预测中为12的个数
        n_gt = np.sum(seg == oid)              # label中为12的个数
        n_intersect = np.sum(np.int32(seg == oid) * mask)       # 交集
        n_union = n_pred + n_gt - n_intersect       # 并联
        iou_log += '_' + str(n_pred)+'_'+str(n_gt)+'_'+str(n_intersect)+'_'+str(n_union)+'_'
        if n_union == 0:
          total_iou += 1
          iou_log += '_1\n'
        else:           # 最后加上这个点云中这类点的Iou
          total_iou += n_intersect * 1.0 / n_union
          iou_log += '_'+str(n_intersect * 1.0 / n_union)+'\n'

      avg_iou = total_iou / len(iou_oids)     # 这个点云的平均iou
      total_acc_iou += avg_iou
      total_per_cat_iou[cur_gt_label] += avg_iou   # 16种点云中每一种点云的iou
      
      if output_verbose:
        output_color_point_cloud(pts, seg, os.path.join(output_dir, str(shape_idx)+'_gt.obj'))   # ground_truth
        output_color_point_cloud(pts, seg_pred_val, os.path.join(output_dir, str(shape_idx)+'_pred.obj'))  # pred
        output_color_point_cloud_red_blue(pts, np.int32(seg == seg_pred_val),  # 预测和标签的差异
    ...
    for cat_idx in range(NUM_OBJ_CATS):
      printout(flog, '\t ' + objcats[cat_idx] + ' Total Number: ' + str(total_per_cat_seen[cat_idx]))   # test过的这类点云的数量
