...
def get_model(point_cloud, is_training, bn_decay=None):
  ...
  adj = tf_util.pairwise_distance(point_cloud[:, :, 6:])            # in:B*N*3 out:B*N*N
  nn_idx = tf_util.knn(adj, k=k) # (batch, num_points, k)           # out:B*N*K
  edge_feature = tf_util.get_edge_feature(input_image, nn_idx=nn_idx, k=k)          # out:B*N*K*18
