...
# models
LOG_MODELS=os.path.join(LOG_DIR, 'models')
if not os.path.exists(LOG_MODELS): os.mkdir(LOG_MODELS)
...
def train():
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    with tf.variable_scope(tf.get_variable_scope()):
      for i in range(FLAGS.num_gpu):  # 遍历gpu
    ...
    for epoch in range(MAX_EPOCH):
      log_string("***Epoch:{}/{}***".format(epoch,MAX_EPOCH))
      sys.stdout.flush()
      ...
      if epoch % 10 == 0:
        save_path = saver.save(sess, os.path.join(LOG_MODELS,'epoch_' + str(epoch)+'.ckpt'))
...
def train_one_epoch(sess, ops, train_writer):
  ...
  for batch_idx in range(num_batches):      # 遍历每一个batch
    ...
    correct = np.sum(pred_val == current_label[start_idx_1:end_idx_1])              # 当前batch中预测正确的个数
    total_correct += correct                              # 本次epoch的正确个数
    total_seen += (BATCH_SIZE*NUM_POINT)                  # 本次epoch的所有点
    loss_sum += loss_val                                  # 本次epoch的总的损失
  
  log_string('mean loss: %f' % (loss_sum / float(num_batches)))       # 本次epoch的平均每一个batch的损失
  log_string('accuracy: %f' % (total_correct / float(total_seen)))    # 本次epoch的平均每一个batch的损失
