        ...
        for batch_idx in range(num_batches):
            ...
            for vote_idx in range(num_votes):
                ...
                loss_val, pred_val = sess.run([ops['loss'], ops['pred']],
                                          feed_dict=feed_dict)
                batch_pred_sum += pred_val          # onehot编码 8*40
            ...
            pred_val = np.argmax(batch_pred_sum, 1)    # 8个点云的类别
            # Aggregating END
            
            correct = np.sum(pred_val == current_label[start_idx:end_idx])   # 8个里面正确的个数
            # correct = np.sum(pred_val_topk[:,0:topk] == label_val)
            total_correct += correct
            total_seen += cur_batch_size
            loss_sum += batch_loss_sum

            for i in range(start_idx, end_idx):  # 遍历当前batch中的每一个点云
                l = current_label[i]
