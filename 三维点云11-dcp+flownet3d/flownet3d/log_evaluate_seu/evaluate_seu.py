...
def eval_one_epoch(sess, ops):
    ...
    for shuffle_cnt in range(SHUFFLE_TIMES):
        ...
        np.savetxt('seu_pcd/f0.txt',batch_data_new[:,0:NUM_POINT,:3].reshape(-1,3))     # f1
        np.savetxt('seu_pcd/f1.txt',batch_data_new[:,NUM_POINT:,:3].reshape(-1,3))      # f2
        np.savetxt('seu_pcd/pred_val.txt',pred_val.reshape(-1,3))                                         # flow_pred
        vis_kitti(batch_data_new[:,0:NUM_POINT,:3].reshape(-1,3),       # 生成点:red,target:green
