...
def get_batch(dataset, idxs, start_idx, end_idx):
    ...
    for i in range(bsize):
        ...
        # # 可视化
        # print('pc1:{},color={},vel:{},mask1:{}'.format(pc1.shape,color1.shape,vel.shape,mask1.shape))
        # vis_cloud(pc1,pc2,color1,color2)
        # vis_cloud1andcolor(pc1,vel)

        # np.savetxt("pc1.txt",pc1)
        # np.savetxt("pc2.txt",pc2)

        # np.savetxt("color1.txt",color1)
        # np.savetxt("color2.txt",color2)
        
        # np.savetxt("vel.txt",vel)
        # np.savetxt("mask1.txt",mask1.reshape(-1,1))
...
def scene_flow_EPE_np(pre, labels, mask):
...
def eval_one_epoch(sess, ops):
    ...
    batch_data = np.zeros((BATCH_SIZE, NUM_POINT*2, 3))     # 1,16384*2,3
    batch_label = np.zeros((BATCH_SIZE, NUM_POINT, 3))
    batch_mask = np.zeros((BATCH_SIZE, NUM_POINT))
    for batch_idx in range(num_batches):
        # print('{}/{}'.format(batch_idx,num_batches))        # 0/150,...
        if batch_idx %20==0:
            log_string('%03d/%03d'%(batch_idx, num_batches))
        start_idx = batch_idx * BATCH_SIZE
        end_idx = min(len(TEST_DATASET), (batch_idx+1) * BATCH_SIZE)        # BATCH_SIZE=1
        ...
        for shuffle_cnt in range(SHUFFLE_TIMES):
            shuffle_idx = np.arange(NUM_POINT)
            np.random.shuffle(shuffle_idx)
            batch_data_new = np.copy(batch_data)            # 两帧点云数据
            batch_data_new[:,0:NUM_POINT,:] = batch_data[:,shuffle_idx,:]       # 前一帧        # (1, 16384, 6) (x,y,z,color,color,color)
            batch_data_new[:,NUM_POINT:,:] = batch_data[:,NUM_POINT+shuffle_idx,:]      # 后一帧
            feed_dict = {ops['pointclouds_pl']: batch_data_new,                 # (1, 16384*2, 6)
                         ops['labels_pl']: batch_label[:,shuffle_idx,:],        # 前一阵的flow  label
                         ops['masks_pl']: batch_mask[:,shuffle_idx],
                         ops['is_training_pl']: is_training}
            loss_val, pred_val = sess.run([ops['loss'], ops['pred']], feed_dict=feed_dict)      # 运行模型，进行推理

            # 保存预测的flow
            print("batch_data_new[:,0:NUM_POINT,:]:{}".format(batch_data_new[:,0:NUM_POINT,:].shape))
            np.savetxt('pred/f1_color.txt',batch_data_new[:,0:NUM_POINT,:3].reshape(-1,3))     # f1
            np.savetxt('pred/f2_color.txt',batch_data_new[:,NUM_POINT:,:3].reshape(-1,3))      # f2
            np.savetxt('pred/label_vel.txt',batch_label[:,shuffle_idx,:].reshape(-1,3))      # flow_label
            np.savetxt('pred/pred_val.txt',pred_val.reshape(-1,3))                                         # flow_pred
